/*
 * serial_send_demo.c - Linux 串口发送示例程序（Hello + 回显）
 *
 * 功能：打开 /dev/ttyACM0（常见于 ESP32 等 USB CDC 串口设备），
 *       以 115200 8N1 配置每秒发送一行 "Hello ESP32 <时间戳>"，
 *       并持续读取串口返回数据并打印到终端。
 *
 * 编译：gcc -O2 -o serial_send serial_send_demo.c
 * 运行：./serial_send
 */
...
int main() {
    /* 打开串口设备（ESP32 等 USB CDC 串口通常为 ttyACM0） */
